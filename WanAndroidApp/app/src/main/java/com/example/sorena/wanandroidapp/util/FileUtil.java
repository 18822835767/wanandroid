...
public class FileUtil
{
    ...
    public static boolean saveFileToSDCardCustomDir(byte[] data, String dir,
        ...
        if (isSDCardMounted()) {
            ...
            if (!file.exists()) {
                LogUtil.d("日志:saveFileToSDCardCustomDir:",file.getAbsolutePath());
                file.mkdirs();// 递归创建自定义目录
            }
            try {
                /*在FileOutputStream的构造器里多加一个参数可以从文件尾追加*/
                bos = new BufferedOutputStream(new FileOutputStream(new File(
                        file, fileName)));
                bos.write(data);
                bos.flush();
     ...
     * @return 文件中的字符串
     */
    public static String getStringInFile(String path,String fileName){

        File file = new File(getSDCardBaseDir() + path,fileName);
        if (!file.exists()){
            LogUtil.d("日志:FileUtil","文件的目录为:" + file.getAbsolutePath());
            return "";
        }
        try {
            ...
            e.printStackTrace();
            LogUtil.e("日志:FileUtil","文件错误:" + e.getMessage());
        }catch (Exception e){
            e.printStackTrace();
            LogUtil.e("日志:FileUtil",e.getMessage());
        }
        return "";
    }
